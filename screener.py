...
api_session.headers.update({
    "User-Agent": "CryptoScreenerBot/2.0 (Mozilla/5.0; YF Trading API System)",
...
def get_histoday_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูล 1D แกนกลางเพียงครั้งเดียวจาก Yahoo Finance"""
    ticker = f"{coin}-USD"
    try:
        df = yf.download(ticker, period="5y", interval="1d", progress=False)
        
        if df.empty:
            logger.warning(f"{coin} 1D: ไม่พบข้อมูลจาก Yahoo Finance")
            return None
        ...
        logger.warning(f"{coin} 1D (YF Error): {e}")
        return None

def get_historical_data(coin: str) -> pd.DataFrame | None:
    """ดึงข้อมูล 1H จาก Yahoo Finance นำมา Resample เป็น 4H"""
    ticker = f"{coin}-USD"
...
def analyze_weekly_context(df_1d: pd.DataFrame) -> dict:
    ...
    try:
        ...
        logger.error(f"Error calculating Weekly Context: {e}")
...
def analyze_onchain_momentum(coin: str) -> dict:
    """Dummy function สำหรับ On-Chain เนื่องจาก Yahoo Finance ไม่มีข้อมูลนี้"""
    return {
        "onchain_warning": False,
        "onchain_label": "🔹 On-Chain: ไม่มีข้อมูล (ใช้ API ของ Yahoo Finance)",
...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ใช้ Wilder's smoothing (com = period - 1) ให้สอดคล้องกับวิธีคำนวณ RSI/ADX ในไฟล์เดียวกัน
    ...
    # FIX: np.where() คืนค่าเป็น numpy array ที่ไม่มี index ติดมา ต้องกำหนด index=df.index
    # ให้ตรงกับ atr_safe (DatetimeIndex) ไม่งั้นตอนหารกัน pandas จะ align index ไม่เจอ
    # แล้วได้ NaN ทั้งคอลัมน์ -> ADX จะกลายเป็น 0 เสมอ (บั๊กเดิม)
...
def calculate_correlation(df: pd.DataFrame, btc_df: pd.DataFrame) -> float:
    """คำนวณ correlation จาก % returns ไม่ใช่ราคาดิบ เพราะราคาดิบที่เทรนด์เดียวกัน
    (เช่น ขาขึ้นพร้อมกัน) จะให้ค่า correlation สูงเทียมแทบทุกเหรียญ ไม่สะท้อนความสัมพันธ์จริง"""
    if btc_df is None or df is None: return 0.0
    try:
        aligned = df["close"].align(btc_df["close"], join="inner")
        if len(aligned[0]) < 3: return 0.0
        returns_coin = aligned[0].pct_change().dropna()
        returns_btc = aligned[1].pct_change().dropna()
        if len(returns_coin) < 2: return 0.0
        corr_val = returns_coin.corr(returns_btc)
        return round(corr_val, 2) if pd.notna(corr_val) else 0.0
    except:
        return 0.0
...
def analyze_rsi_bounce(df: pd.DataFrame) -> dict:
    ...
    # FIX: เดิมนับจำนวนแท่งขึ้นทั้งหมดในหน้าต่าง ไม่ใช่ "ต่อเนื่อง" จริง (ไม่ break เมื่อเจอแท่งลง)
    # แก้ให้ break ทันทีที่เจอแท่งที่ไม่ขึ้น เพื่อให้เป็น streak ต่อเนื่องจริงๆ เหมือน analyze_trend_continuity
    consec = 0
...
def format_price(price: float) -> str:
    if price is None or math.isnan(price): return "N/A"
...
def scan_market():
    ...
    logger.info("ดึงข้อมูล Bulk Funding Rate จาก Binance...")
    funding_map = {}
    try:
    ...
    for coin in COINS:
        ...
        # FIX: BTC ถูกดึงและคำนวณ indicators ไปแล้วด้านบน (สำหรับ market regime/correlation baseline)
        # ถ้าเหรียญรอบนี้คือ BTC ให้ใช้ btc_df ซ้ำแทนการยิง API ดึงข้อมูลซ้ำซ้อน
        ...
        in_fibo_zone = (weekly_ctx.get("fibo_618") is not None) and (current_price <= weekly_ctx["fibo_618"] * 1.02)
        ...
        # ----------------------------------------------------------------------------------
        # บล็อกเช็กเทรนด์ขาขึ้น/ลง และนับสถิติ แยกออกจากบล็อกตรวจจับสัญญาณช้อนซื้อ
        # ----------------------------------------------------------------------------------
        if current_price > ema_200:
            ...
            # สัญญาณซื้อจะเกิดขึ้นได้ก็ต่อเมื่อ RSI ยังไม่เข้าโซน Overbought เท่านั้น
            if rsi < RSI_OVERBOUGHT:
                if in_fibo_zone or in_4h_fibo_zone or in_ob_zone or in_fvg_zone:
                    # FIX: เดิม (rsi <= RSI_OVERSOLD or rsi <= RSI_PULLBACK_THRESHOLD) ซ้ำซ้อนกัน
                    # เพราะ RSI_OVERSOLD(32) <= RSI_PULLBACK_THRESHOLD(55) อยู่แล้ว เงื่อนไขแรกไม่มีผลจริง
                    # แยกเป็น 2 โซนชัดเจน: Oversold รุนแรง (เข้าได้แม้ bounce ยังไม่ชัด)
                    # กับ Pullback ปกติ (ต้องมี bounce ยืนยันคุณภาพดีเท่านั้นถึงจะเข้า)
                    if current_price > (ema_50 * 0.98):
                        if rsi <= RSI_OVERSOLD:
                            # โซน Oversold รุนแรง: นัยสำคัญสูงอยู่แล้ว ไม่บังคับต้องมี bounce ชัดเจน
                            if bounce_info["quality"] in ["strong", "moderate"]:
                                signal_type = f"Institution Dip & Rebound (Severe Oversold) 📉{vol_tag}"
                            else:
                                signal_type = f"Golden Fib / OB Zone Oversold (Severe) 📉{vol_tag}"
                        elif rsi <= RSI_PULLBACK_THRESHOLD:
                            # โซน Pullback ปกติ: ต้องมี bounce ยืนยันคุณภาพ strong/moderate เท่านั้น
                            if bounce_info["quality"] in ["strong", "moderate"]:
                                signal_type = f"Institution Dip & Rebound (Normal Pullback) 📉{vol_tag}"
        ...
        # ----------------------------------------------------
        # การจัดเก็บสัญญาณ: ใช้ if...elif ป้องกันส่งสัญญาณชนกัน
        # ----------------------------------------------------
        if signal_type:
        ...
        # ให้ตรวจสอบและส่งสัญญาณขาย เฉพาะกรณีที่ไม่ติดสัญญาณซื้อเท่านั้น
        elif rsi >= RSI_OVERBOUGHT or ob_info["has_bearish_ob"]:
...
if __name__ == "__main__":
    logger.info("เริ่มต้นใช้งาน Crypto Screener (SMC + YF + Binance Futures API)...")
